...
void parent_code(int pipe_to_child[], int pipe_to_parent[])
{
    ...
    // close the write end of the pipe to parent
...
void child_code(int pipe_to_child[], int pipe_to_parent[])
{
    ...
    // close the read end of the pipe to parent
    ...
    // write to pipe_from_child
    char resp[] = "I love you, parent.";
    size = write(pipe_to_parent[1], resp, strlen(resp)+1);
    if (size == -1)
...
int main(int argc, char *argv[])
{
    ...
    // we don't need to wait for the child because the pipe read
    // in the parent is blocking -- it will wait until the child
