...
{
    return hash_int(ip) == hash_int(jp);
}
...
Value *list_lookup(Node *curr, Hashable *key)
{
    while(curr != NULL) {
        if(equal_hashable(key, curr->key)) return curr->value;
        curr = curr->next;
    }
...
{
    int idx = hash_hashable(key) % map->n;
    return list_lookup(map->lists[idx], key);
}
